...
(function () {
   window.GLPIKanban = function () {
       ...
       * @type {string}
       */
      this.newtasklists_root = CFG_GLPI.root_doc + "/plugins/tasklists/ajax/";
      // this.tasklists_root = CFG_GLPI.root_doc + "/plugins/tasklists/ajax/";
      this.tasklists_root = CFG_GLPI.root_doc + "/ajax/";
      // this.root_doc = CFG_GLPI.root_doc + "/ajax/";
...
//INFOTEL
      var buildToolbar = function () {
         ...
         $.ajax({
            method: 'GET',
            url: (self.newtasklists_root + "addOptions.php"),
...
//TODO INFOTEL
      ...
      const registerEventListeners = function() {
         ...
         $(self.element + ' .kanban-toolbar').on('select2:select', switcher, function(e) {
            ...
            $.ajax({
               type: "GET",
               url: (self.newtasklists_root + "kanban.php"),
               data: {
                  action: "get_url",
...
//INFOTEL
      ...
      //
      //
      //         + '     var href ="' + self.root_doc + '/ajax/contextForm.php?newContext=1";'
      ...
      //       <li id="${card['id']}" class="kanban-item card ${readonly ? 'readonly' : ''} ${card['is_deleted'] ? 'deleted' : ''}">`;
         ...
         //    card_el +=`<div class="kanban-item-header">
         //            <span class="kanban-item-title" title="${card['title_tooltip']}">
         //            <i class="${self.supported_itemtypes[itemtype]['icon']}"></i>
         //                ${card['title']}
         //            </span>
         ...
         if (card.finished == 1 && card.archived == 0) {
            link += '<a id="archivetask' + card.id + '" href="#" title="' + self.translations['archive_task'] + '"><i class="faq fa-archive"></i></a>&nbsp;';
         }
