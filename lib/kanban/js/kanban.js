...
(function(){
    window.GLPIKanban = function() {
        ...
        var buildToolbar = function() {
            ...
            var option = $("<div id='opt-kanban' class='kanban'></div>").appendTo(self.element);
            var archive = $("<div><div>"+self.translations["status"]+" : </div><div id='opt-archive' class='kanban'></div></div>").appendTo(option);
            var users = $("<div><div>"+self.translations["users"]+" : </div><div id='opt-users' class='kanban'></div></div>").appendTo(option);
            ...
            $.ajax({
                ...
                $(data).appendTo(archive);
                    ...
                    window.console.log("zzest parti");
        ...
        var fillColumns = function(columns_container) {
            ...
            $.each(self.columns, function(column_id, column) {
                if (!already_processed.includes(column_id)) {
                    ...
                    window.console.log(columns_container);
        ...
        var registerEventListeners = function() {
           ...
           */
            ...
            $(self.element).on('click', '.kanban-add-column', function() {
                window.console.log("add collum button");
          ...
          /*  $(self.add_column_form).on('click', '.kanban-create-column', function() {
            ...
            $(self.create_column_form).on('click', '.kanban-create-column', function() {
                //TODO changer le fonctionnement de bouton de création de colonne
                var toolbar = $(self.element + ' .kanban-toolbar');
                $(self.create_column_form).css({
                    display: 'none'
                });
            ...
            $(self.element + ' .kanban-toolbar').on('select2:select', switcher, function(e) {
                ...
                window.console.log("changement switch");
                window.console.log(items_id);
            ...
            bodies.sortable({
                ...
                update: function(event, ui) {
                    if (this === ui.item.parent()[0]) {
                        window.console.log("kanbanCardSort");
        ...
        var getTeamBadge = function(teammember) {
            ...
            if (self.team_badge_cache[itemtype] === undefined ||
                ...
                if (itemtype === 'User') {
                    var user_img = null;
                    $.ajax({
                        url: (self.tasklists_root + "getUserPicture.php"),
                        async: false,
                        data: {
                            users_id: [items_id],
...
*/
            ...
            card_el += "<div class='kanban-item-header'>"+link+"<a href='"+card['link']+"'>" + card['title'] + "</a>"+card.user+"</div>";
            card_el += "<div class='kanban-item-client'>" + card['client'] + "</div>";

            card_el += "<div class='kanban-item-content'>" + (card['description'] || '') + "</div>";
            if (card.right == 1) {
                    ...
                    $(document).ready(function() {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                var items_id = ele;
                                window.console.log(items_id);
                                // $(ele + ' .kanban-toolbar').on('select2:select', switcher, function(e) {
                                //     var items_id = ele.params.data.id;
                                //     window.console.log(ele.data);
                                // });
