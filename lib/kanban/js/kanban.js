...
(function () {
    window.GLPIKanban = function () {
         ...
         * @type {string}
         */
        this.tasklists_root = PLUGIN_TASKLISTS_WEBDIR + "/ajax/";
        this.root_doc = PLUGIN_TASKLISTS_WEBDIR;
        ...
        var updateCardPosition = function (card, column, position, error, success) {
            ...
            var d = {action: "move_item",card: card,column: column,position: position,kanban: self.item};
            d = JSON.stringify(d);
            $.ajax({
                type: "POST",
                url: (self.tasklists_root + "kanban.php"),
                data: d,
                processData: false,
                // data: {action: "move_item",card: card,column: column,position: position,kanban: self.item},
                dataType: 'json',
                // contentType: 'application/json',
        ...
        var appendColumn = function (column_id, column, columns_container, revalidate) {
            ...
            if (self.allow_add_item) {
                ...
                $(column_right).append('<script>'
                    ...
                    + '     var href ="' + self.root_doc + '/ajax/seetask.php?plugin_tasklists_taskstates_id='
