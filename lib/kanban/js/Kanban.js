 ...
 */
//INFOTEL
import SearchInput from "../../../../../js/modules/SearchTokenizer/SearchInput";
   ...
   window.GLPIKanban = function() {
      ...
      const initParams = function(args) {
         const overridableParams = [
            'root_taslists','element', 'max_team_images', 'team_image_size', 'item',
      ...
      **/
      ...
      const buildToolbar = function() {
         ...
         //Infotel
         // var archive = $("<div id='opt-archive'></div>").appendTo(toolbar);
         if (self.rights.canModifyView()) {
         ...
         // $.ajax({
         //    method: 'GET',
         //    url: (self.tasklist_root + "addOptions.php"),
         //
         //    data: {
         //       action: "addArchived",
         //    },
      ...
      const registerEventListeners = function() {
         ...
         //Infotel
         // var switcharchive = $("select[name='archive[]']").first();
         // $('#opt-archive').on('change', switcharchive, function (e) {
         //
         //    var values = [];
         //
         ...
         $(self.element + ' .kanban-toolbar').on('select2:select', switcher, function(e) {
            ...
            $.ajax({
               type: "GET",
//INFOTEL
               url: (self.tasklist_root + "kanban.php"),
               data: {
                  action: "get_url",
...
//INFOTEL
         add_form += "<input type='hidden' name='plugin_tasklists_tasktypes_id' value='" + self.item.items_id + "'/>";
         add_form += "<input type='hidden' name='" + self.column_field.id + "' value='" + column_value + "'/>";
         add_form += "<input type='submit' value='" + __('Add') + "' name='add' class='submit btn btn-primary'/>";
         add_form += "</form>";
      ...
      const buildAddColumnForm = function() {
         ...
         if (self.rights.canCreateColumn()) {
            add_form += `
               ...
               <button role='button' class='submit btn btn-primary kanban-create-column d-block'>${__('Create status')}</button>
            `;
         }
         add_form += "</form></div>";
      ...
      const buildCreateColumnForm = function() {
         ...
         create_form += "</div>";
         create_form += "<button type='submit' class='submit btn btn-primary'>" + __('Create status') + "</button>";
      ...
      const appendCard = function(column_el, card, revalidate = false) {
         ...
         //Infotel
         const items_id = card['id'].split('-')[1];
         ...
         card_el += link;
         card_el += "</span>";
         card_el += "<i class='kanban-item-overflow-actions fas fa-ellipsis-h btn btn-sm btn-ghost-secondary'></i>";
         card_el += "</div>";
         card_el += `<div class="kanban-item-content">${(card['content'] || '')}</div>`;
         //Infotel
         card_el += "<div class='kanban-item-content b'>" + card['client'] + "</div>";
         card_el += '<div align="right" class="endfooter">' + card.actiontime + '</div>';
         if (card.duedate) {
            card_el += '<div style="background-color:'
               + card.bgcolor + '" class="cd_kanban_board_block_item_priority">' + card.duedate + '</div>';
         }
         if (card.right == 1) {
            card_el += '<div class=" cd_kanban_board_block_item_footer' + '">' +
               '<br><div align="center"><div class="kanban_slider" id="slider' + items_id + '"></div></div>';
...
//Infotel
         $(document).ready(function () {
            ...
            $("#percent" + items_id).val($("#slider" + items_id).slider("value") + "%");
         });
      };
    ...
    *    @param {boolean} revalidate If true, all other columns are checked to see if they have an item in this new column.
      ...
      this.showCardPanel = (card) => {
         ...
         $.ajax({
            ...
            $('.item-details-panel ul.team-list li').each((i, l) => {
               ...
               l.append(`
                     ...
                     <button type="button" name="delete" class="submit btn btn-ghost-danger">
         ...
         $(self.element).on('click', '.item-details-panel ul.team-list button[name="delete"]', (e) => {
            ...
            if (itemtype && items_id) {
               //Correction Infotel
               removeTeamMember(itemtype, items_id, member_itemtype, member_items_id, 2);
               list_item.remove();
            }
         });
      };
      ...
      this.showTeamModal = (card_el) => {
         ...
         content += `
            ...
            <button type="button" name="add" class="submit btn btn-primary">${_x('button', 'Add')}</button>
         `;
         const modal = $('#kanban-modal');
