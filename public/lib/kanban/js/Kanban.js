      ...
      **/
       ...
       */
        const fillColumns = function(columns_container) {
            ...
            $.each(self.user_state.state, function(position, column) {
                if (column != null && column['visible'] !== false && column !== 'false') {
       ...
       */
        const getColumnToolbarElement = function(column) {
            ...
            //Infotel add link for all archive
            ...
            $.ajax({
                ...
                complete: function() {
                    $.each(self.user_state.state, function(i, c) {
                        if (c != null && parseInt(c['column']) === parseInt(column)) {
      ...
      **/
       ...
       */
        const appendColumn = function(column_id, column, columns_container, revalidate) {
            ...
            $.each(self.user_state.state, function(order, s_column) {
                if (s_column != null && parseInt(s_column['column']) === parseInt(column_id)) {
                    position = order;
            ...
            $(document).ready(function () {
                $("i#kanban_delete_" + column['id']).click(function () {
                    var archivealltasks = 1;
                    var state_id = column_id;
       ...
       */
        const appendCard = function(column_el, card, revalidate = false) {
            ...
            //Infotel add field or update it
            ...
            //Infotel add  style="background-color:${card['bgcolor']}"
            //Infotel add archive & update priority links
            let link = "";
            ...
            if (card.finished == 0 && card.priority_id < 5) {
            ...
            <li id="${card['id']}" class="kanban-item card ${readonly ? 'readonly' : ''} ${card['is_deleted'] ? 'deleted' : ''}" style="background-color:${card['bgcolor']};border-color:${card['bordercolor']}">
                <div class="kanban-item-header">
                    <span class="kanban-item-title" title="${escapeHtml(card['title_tooltip'])}">
                    <i class="${self.supported_itemtypes[itemtype]['icon']}"></i>
                        ${card['title']}`
            card_el += link;
            card_el += `</span>
         ...
         `;
            ...
            //Infotel - add click to archive & update priority
            $(document).ready(function () {
    ...
    *    @param {boolean} revalidate If true, all other columns are checked to see if they have an item in this new column.
       ...
       */
        const loadColumn = function(column_id, nosave, revalidate, callback = undefined) {
            ...
            let skip_load = false;
            $.each(self.user_state.state, function(i, c) {
                if (c != null && parseInt(c['column']) === parseInt(column_id)) {
                    if (!c['visible']) {
                        skip_load = true;
                    }
                    return false;
